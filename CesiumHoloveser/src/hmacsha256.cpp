#include "CesiumHoloveser/hmacsha256.h"
...
PBKDF2_SHA256_DEF void sha256_final(SHA256_CTX* s, uint8_t* md) {
  ...
  s->len *= 8;
  s->buf[56] = (s->len >> 56) & 0xff;
  s->buf[57] = (s->len >> 48) & 0xff;
  s->buf[58] = (s->len >> 40) & 0xff;
  s->buf[59] = (s->len >> 32) & 0xff;
  s->buf[60] = (s->len >> 24) & 0xff;
  s->buf[61] = (s->len >> 16) & 0xff;
  s->buf[62] = (s->len >> 8) & 0xff;
  s->buf[63] = (s->len) & 0xff;
  sha256_transform(s, s->buf);

  for (size_t j = 0; j < SHA256_DIGESTINT; j++) {
    md[4 * j] = s->h[j] >> 24;
    md[4 * j + 1] = (s->h[j] >> 16) & 0xFF;
    md[4 * j + 2] = (s->h[j] >> 8) & 0xFF;
    md[4 * j + 3] = (s->h[j]& 0xFFFF) & 0xFF;;
  }
  sha256_init(s);
}

PBKDF2_SHA256_DEF void
sha256_update(SHA256_CTX* s, const uint8_t* m, uint32_t len) {
  const uint8_t* p = m;
  uint32_t r = s->len % SHA256_BLOCKLEN;
...
PBKDF2_SHA256_DEF void
hmac_sha256_init(HMAC_SHA256_CTX* hmac, const uint8_t* key, uint32_t keylen) {
  ...
  if (keylen <= SHA256_BLOCKLEN) {
    ...
    memset(
        hmac->buf + SHA256_DIGESTLEN,
        '\0',
        SHA256_BLOCKLEN - SHA256_DIGESTLEN);
  }
...
PBKDF2_SHA256_DEF void
hmac_sha256_update(HMAC_SHA256_CTX* hmac, const uint8_t* m, uint32_t mlen) {
...
PBKDF2_SHA256_DEF void pbkdf2_sha256(
...
/**/
std::string hmacSha256( const char *SignData, const char *szSecret)
{
    ...
    const uint8_t d1=static_cast<uint8_t>(std::stoul(szSecret));
    const uint8_t d2=static_cast<uint8_t>(std::stoul(SignData));

    hmac_sha256_init(&hmac, &d1, static_cast<uint32_t>(strlen((char *)szSecret)));
    hmac_sha256_update(&hmac,&d2 , static_cast<uint32_t>(strlen((char *)SignData)));
    ...
    std::string strSign;
    for(size_t i = 0; i < SHA256_DIGESTLEN; i++)
    {
